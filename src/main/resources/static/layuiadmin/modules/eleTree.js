...
layui.define(["jquery","laytpl"], function (exports) {
    ...
    var thisTree=function() {
        ...
        return {
            ...
            expandFirst: function() {
                options.elem.children(".eleTree-node").children(".eleTree-node-group").empty();
                _self.expandFirst.call(_self,options.data,[],1,true);
                _self.unCheckNodes(true);
                _self.defaultChecked();
                _self.checkboxInit();
            },
        ...
        return [
            '{{# for(var i=0;i<d.length;i++){ }}',
                '<div class="eleTree-node {{# if(d[i].visible===false){ }}eleTree-search-hide{{# } }}" data-'+options.request.key+'="{{d[i]["'+options.request.key+'"]}}" eletree-floor="'+floor+'" style="display: none;">',
                    '<div class="eleTree-node-content" style="padding-left: '+(options.indent*floor)+'px;width: 80%;overflow: hidden;text-overflow:ellipsis;">',
                        ...
                        '<span class="eleTree-node-content-label" title={{d[i]["'+options.request.name+'"]}}>{{d[i]["'+options.request.name+'"]}}</span>',
                    '</div>',
    ...
    Class.prototype={
        ...
        config: {
            ...
            customSelection : false,    //自定义选择节点（不遵循父子互相关联情况时）
        ...
        checkboxEvent: function() {
            ...
            options.elem.on("click",checkOnClickNode,function(e) {
                ...
                if(inp.checked){
                    ...
                    // 点击祖父层选中子孙层
                    if(!options.checkStrictly || options.customSelection){
        ...
        expandFirst: function(data,arr,floor,isMethodsExpandAll) {
            var options=this.config;
            var _self=this;
            try {
                data.forEach(function(val,index) {
